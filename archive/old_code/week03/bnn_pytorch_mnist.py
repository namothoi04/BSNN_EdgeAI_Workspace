...
class BinaryActivation(nn.Module):
    def forward(self, x):
        # Ép về 0 và 1 
        out = torch.where(x >= 0, torch.tensor(1.0, device=x.device), torch.tensor(-1.0, device=x.device))
        # Áp dụng STE để gradient có thể chảy qua
...
class BinaryCNN(nn.Module):
    def __init__(self):
        ...
        # Dùng lớp tự định nghĩa (in_channels=1 cho ảnh đen trắng, 32 filters)
        self.conv1 = BinaryWeightConv2d(in_channels=1, out_channels=32, kernel_size=3)
        self.bin_act = BinaryActivation()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.flatten = nn.Flatten()
        
        # Tính toán kích thước đầu vào cho lớp Dense (Linear)
        # Ảnh 28x28 -> qua Conv2D (kernel 3) còn 26x26 -> qua Pool (kernel 2) còn 13x13
        self.fc1 = nn.Linear(32 * 13 * 13, 10)

    def forward(self, x):
        x = self.conv1(x)
        # x = F.relu(x)        # 5. Đi qua hàm kích hoạt Float
        x = self.bin_act(x)
        x = self.pool(x)
        x = self.flatten(x)
        x = self.fc1(x)
        return x             # Không dùng Softmax ở đây vì CrossEntropyLoss đã bao hàm
